...
class MLP(nn.Module):
    def __init__(
        ...
        self.layers = layers
        self.positional_embeddings = nn.Sequential(*positional_embeddings)
    ...
    def forward(self, t, x, x_self_cond=False):
        ...
        #print(self.positional_embeddings(x).shape, t_emb.shape)
...
class MyMLPNoSpaceEmbedding(nn.Module):
    def __init__(
        ...
        self.channels = 1
        self.self_condition = False
        concat_size = len(self.time_mlp.layer) + input_dim
        layers = [nn.Linear(concat_size, hidden_size)]
        for _ in range(hidden_layers):
            layers.append(Block(hidden_size, emb_size, add_t_emb, concat_t_emb))
...
class TorchWrapper(nn.Module):
    ...
    def forward(self, x, t):
        # Falls t schon die richtige Batch-Form hat (B, 1), nutzen wir es direkt.
        # Falls es nur ein Skalar ist, machen wir es passend.
        if t.dim() == 0:
            time = t.view(1, 1).repeat(x.shape[0], 1).to(device=x.device, dtype=x.dtype)
        elif t.dim() == 1:
            # Falls t Form (B,) hat, machen wir (B, 1) draus
            time = t[:, None].to(device=x.device, dtype=x.dtype)
        else:
            # t hat bereits die richtige Form (B, 1)
            time = t.to(device=x.device, dtype=x.dtype)
            
        labels = self._labels
        if labels is not None:
            if labels.shape[0] != x.shape[0]:
                raise ValueError("Mismatch between stored labels and batch size")
