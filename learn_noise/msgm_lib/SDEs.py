...
class SDE(torch.nn.Module):
    ...
    def sample_scheme(self, t, y0, return_noise=False):
        """
        sample yt | y0 (VECTORIZED FOR GPU)
        """
        if return_noise:
        ...
        # 1. Generate the full trajectory (Now safely staying on the GPU!)
        y_allt = self.sample_scheme_allt(y0, include_t0=include_t0)

        # 2. Calculate the correct time step index for each batch item
        num_steps_floats = num_steps_tot * t / self.T
        
        # Ensure it stays on the device and squeeze it to 1D
        num_steps_int = torch.trunc(num_steps_floats).to(torch.long).squeeze()
        num_steps_int = num_steps_int.to(y0.device)

        # 3. Clamp indices so they don't crash if t >= T
        max_idx = y_allt.shape[0] - 1
        safe_indices = torch.clamp(num_steps_int, 0, max_idx)

        # 4. VECTORIZED GATHER!
        # This replaces the horrific "for k in range(y0.shape[0]):" loop.
        # It instantly grabs the correct timestep for all 2000 items at once.
        batch_idx = torch.arange(y0.shape[0], device=y0.device)
        yt = y_allt[safe_indices, batch_idx, :]
