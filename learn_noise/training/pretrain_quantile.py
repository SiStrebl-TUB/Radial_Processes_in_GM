...
def pretrain_quantile(args, *, log_step_offset: int = 0) -> Tuple[nn.Module, str, int]:
    ...
    quantile = build_quantile(args, device, 1) #dim
    ...
    for step in tqdm(range(total_steps), desc="Quantile training (ot)"):
        ...
        rand_u = torch.rand(B, 1, device=device, dtype=x0.dtype)
        ...
        uni = torch.randn(size = (B, args.dim), device = device)
        ...
        if step in target_steps:
            ...
            # =================================================================
            # 1. Daten generieren (Radien)
            # =================================================================
            u_test = torch.linspace(0.01, 0.99, 200000, device=device).reshape(200000, 1)
            t_test = torch.ones(200000, 1, device=device)
            eps_test, q_test = quantile(u_test, t_test, return_dqdt=True, requires_grad=False)

            u_np = u_test.detach().cpu().numpy()[::100, 0]
            q_np = q_test.detach().cpu().numpy()[::100, 0]
            quantile_history[step] = (u_np, q_np)

            # =================================================================
            # 2. 2D Samples generieren (Radius * Uniform Direction)
            # =================================================================
            # Zufällige Richtungen auf der 2D-Sphäre (Kreis) generieren
            uni_test = torch.randn(size=(200000, 2), device=device)
            uni_test = torch.nn.functional.normalize(uni_test, dim=1)
            # 2D Koordinaten = Radius * Richtung
            samples_2d = q_test * uni_test
            samples_np = samples_2d.detach().cpu().numpy()

            # 3. Den Plot erstellen (1x2 Subplot)
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6), gridspec_kw={'width_ratios': [1.2, 1]})
            
            # Farbpalette anpassen an Script 1
            custom_colors = ["#1a68cf", "#1acf47", "#d62828", "#13C92C", "#1a68cf"]
            custom_cmap = mcolors.LinearSegmentedColormap.from_list("custom_bunt", custom_colors)
            colors = custom_cmap(np.linspace(0, 1, len(target_steps)))
            current_step_idx = target_steps.index(step)
            current_color = colors[current_step_idx]

            # =================================================================
            # PLOT 1: Die Quantilfunktion (Linke Seite)
            # =================================================================

            for idx, (past_step, (u_val, q_val)) in enumerate(quantile_history.items()):
                is_current = (past_step == step)
                # Linienbreiten und Alphas an Script 1 angepasst
                lw = 2.5 if is_current else 1.5
                alpha = 1.0 if is_current else 0.4
                ax1.plot(u_val, q_val,
                        color=colors[idx],
                        linewidth=lw,
                        alpha=alpha,
                        linestyle='-',
                        label=r"$x_1$, $x_2$" if is_current else None)

            ax1.set_title("Radial Quantile Function", fontsize=22, pad=15)
            ax1.set_xlabel(r"$u \sim \mathcal{U}(0,1)$", fontsize=20)
            ax1.set_ylabel(r"$\mathbf{Q}_{\phi}^{\mathrm{rad}}(u)$", fontsize=20)
            ax1.set_xlim(0, 1)
            ax1.set_ylim(0, 6)
            ...
            # =================================================================
            # PLOT 2: Der resultierende 2D Latent Space (Rechte Seite)
            # =================================================================
            # Scatterplot auf Format aus Script 1 gesetzt
            ax2.scatter(samples_np[:, 0], samples_np[:, 1], s=3, alpha=0.5, color=current_color, edgecolors='none')

            ax2.set_title("Resulting 2D Latent Space", fontsize=22, pad=15)
            # Achsenbeschriftungen hinzugefügt
            ...
            # =================================================================
            # 5. Speichern und in WandB loggen
            # =================================================================
            if args.runs_dir:
                output_dir = os.path.join(args.runs_dir, "quantile_plots")
                os.makedirs(output_dir, exist_ok=True)
                save_path = os.path.join(output_dir, f"quantile_step_{step}.png")
                fig.savefig(save_path, dpi=300)
            
            # Try/Except Block für WandB hinzugefügt
            try:
                wandb.log({"eval/quantile_function_evolution": wandb.Image(fig)}, step=global_step)
            except Exception:
                pass
                
            plt.close(fig)

        x0_norm_signed = torch.linalg.vector_norm(x0, dim=1)#*torch.randn(x0.shape[0], 1).sign()
        x0_norm_signed = x0_norm_signed.reshape(-1, 1)
        transport_loss, transport_metrics = transport_objective(x0_norm_signed, dqdt_1d)
        reg_logdet = (
