...
COL_BG_DARK  = "#E9EDF2"
COL_PATH     = "#4DD83B"# "#25BA12"#"#4682B4"
COL_PATH_OUTLIER = "#FF6B35"  # orange-red for outlier trajectories
...
def _get_lowdim_limits(args) -> tuple[float, float, float, float]:
    ...
    elif dataset.lower() == "unigauss":
        return -100.0, 100.0, -100.0, 100.0
    elif dataset.lower() == "anisotropicgauss":
        return -150.0, 150.0, -150.0, 150.0
    elif dataset.lower() == "radialpareto":
...
def plot_tail_ccdf(args, generated_radii, r_min=2.0, alpha=1.5, step=0, filename=None, key="tail_ccdf") -> None:
    """
    Erstellt einen Log-Log CCDF Plot, um das Power-Law-Verhalten der Tails zu prüfen.
    generated_radii: 1D Numpy Array der Radien deiner generierten Samples (z.B. np.linalg.norm(ends, axis=1))
    """
    # 1. Empirische CCDF der generierten Daten berechnen
    sorted_radii = np.sort(generated_radii)
    n = len(sorted_radii)
    # y-Werte: Wahrscheinlichkeit P(R > r) -> 1, 0.999, ..., 1/n
    empirical_ccdf = np.arange(n, 0, -1) / n

    # 2. Theoretische CCDF der Pareto-Verteilung
    # Wir plotten die Theorie nur für den Bereich, den wir auch gesampelt haben
    r_theory = np.geomspace(r_min, 1000, 100)
    theoretical_ccdf = (r_min / r_theory) ** alpha
    ...
    plt.plot(sorted_radii, empirical_ccdf, marker='.', linestyle='none', 
             color='#d62728', alpha=0.7, label='Generated Samples')
    
    # Theoretische Gerade
    plt.plot(r_theory, theoretical_ccdf, color='#1f77b4', linewidth=2.5, 
             linestyle='--', label=f'Target Pareto Tail ($\\alpha={alpha}$)')

    plt.xscale('log')
    plt.yscale('log')
    plt.xlabel(r'Radius $x$ (Log Scale)', fontsize=16)
    plt.ylabel(r'$\text{Pr}(X > x)$ (Log Scale)', fontsize=16)
...
def _render_trajectory_panel(
    *,
    ...
    Args:
        outlier_indices: Optional array of indices marking which trajectories are outliers.
                        These will be colored differently (COL_PATH_OUTLIER instead of COL_PATH).
    """
    x_min, x_max, y_min, y_max = _get_lowdim_limits(args)
    ...
    # Create outlier set for O(1) membership testing
    ...
    for idx in range(starts.shape[0]):
        if idx in outlier_set:
            ax.plot(trajectories[:, idx, 0], trajectories[:, idx, 1], **line_kwargs_outlier)
        else:
            ax.plot(trajectories[:, idx, 0], trajectories[:, idx, 1], **line_kwargs_normal)
    ...
    with torch.inference_mode():
        if do_light:
            if hasattr(ode_func, 'reset_nfe'):
                ode_func.reset_nfe()
            num_traj = min(100000, args.eval_sample)
            if num_traj > 0:
                # Generate fixed noise using deterministic seed for consistent evaluation
                gen = torch.Generator(device=device)
                gen.manual_seed(args.seed + 999)  # Fixed seed offset for eval noise
                if args.baseline_flow == "target_norm":
                    x0 = sampler.sample(num_traj, device=device, dtype=torch.float32)
                    eps1 = noise_sampler((num_traj, args.dim), x0).to(device) #args.dim
                else:
                    # Use seeded generator for reproducible noise
                    noise_shape = (num_traj, args.dim)
                    eps1_raw = torch.randn(noise_shape, generator=gen, device=device, dtype=torch.float32)
                    # Scale by sampler if available, otherwise use raw noise
                    if hasattr(noise_sampler, '__self__') or callable(noise_sampler):
                        try:
                            # Try to pass seed_offset for samplers that support it
                            eps1 = noise_sampler(noise_shape, seed_offset=999).to(device)
                        except TypeError:
                            # Fallback for samplers without seed_offset parameter
                            eps1 = noise_sampler(noise_shape).to(device)
                    else:
                        eps1 = eps1_raw
                ...
                if args.target_dataset == 'funnel':
                    xlim = (-20.0, 20.0)
                    ylim = (-100.0, 100.0)
                elif args.target_dataset == 'anisotropicgauss':
                    xlim = (-150.0, 150.0)
                    ylim = (-150.0, 150.0)
                else:
                    xlim = (-4.0, 4.0)
                ...
                if n_paths > 0:
                    ...
                    if args.slerp:
                        ...
                        x_traj = spherical_ode_solver(ode_func, noise, t_vals)
                        x_traj = x_traj / (scale+1e-8)  # Rückskalierung der Trajektorie auf lineare Geschwindigkeit
                    else:
                        x_traj = odeint(ode_func, eps1[:n_paths], t_vals, method='dopri5')

                    X = x_traj.detach().cpu().numpy() # scale back to original distribution
                    starts = X[0]  # scale back to original distribution
                    ends = X[-1] # scale back to original distribution
                    
                    # Identify outlier trajectories (top 2% by norm of starting noise)
                    eps1_norms = torch.linalg.norm(eps1[:n_paths], dim=1).cpu().numpy()
                    outlier_percentile = 0.1
                    outlier_count = max(1, int(n_paths * outlier_percentile / 100.0))
                    outlier_threshold = np.percentile(eps1_norms, 100 - outlier_percentile)
                    outlier_indices = np.where(eps1_norms >= outlier_threshold)[0]
                    
                    # Detailed logging - INPUT NOISE
                    outlier_norms_input = eps1_norms[outlier_indices]
                    normal_norms_input = eps1_norms[~np.isin(np.arange(n_paths), outlier_indices)]
                    
                    print(f"[Baseline @ step {step}] INPUT NOISE DISTRIBUTION")
                    print(f"  Total: {n_paths} | Outliers: {len(outlier_indices)} ({100*len(outlier_indices)/n_paths:.2f}%) | Normal: {n_paths - len(outlier_indices)}")
                    print(f"  Outlier norms - Mean: {outlier_norms_input.mean():.4f} | Std: {outlier_norms_input.std():.4f} | Min: {outlier_norms_input.min():.4f} | Max: {outlier_norms_input.max():.4f}")
                    print(f"  Normal norms  - Mean: {normal_norms_input.mean():.4f} | Std: {normal_norms_input.std():.4f} | Min: {normal_norms_input.min():.4f} | Max: {normal_norms_input.max():.4f}")
                    print(f"  Threshold: {outlier_threshold:.4f} | Gap (outlier - normal mean): {(outlier_norms_input.mean() - normal_norms_input.mean()):.4f}")
                    ...
                    ends_norms = np.linalg.norm(ends, axis=1)
                    outlier_norms_output = ends_norms[outlier_indices]
                    normal_norms_output = ends_norms[~np.isin(np.arange(n_paths), outlier_indices)]
                    
                    print(f"\n[Baseline @ step {step}] GENERATED OUTPUT DISTRIBUTION")
                    print(f"  Outlier norms - Mean: {outlier_norms_output.mean():.4f} | Std: {outlier_norms_output.std():.4f} | Min: {outlier_norms_output.min():.4f} | Max: {outlier_norms_output.max():.4f}")
                    print(f"  Normal norms  - Mean: {normal_norms_output.mean():.4f} | Std: {normal_norms_output.std():.4f} | Min: {normal_norms_output.min():.4f} | Max: {normal_norms_output.max():.4f}")
                    print(f"  Gap (outlier - normal mean): {(outlier_norms_output.mean() - normal_norms_output.mean()):.4f}")
                    
                    # COMPARISON: Input vs Output
                    print(f"\n[Baseline @ step {step}] INPUT -> OUTPUT TRANSFORMATION")
                    print(f"  Outlier: {outlier_norms_input.mean():.4f} -> {outlier_norms_output.mean():.4f} (Δ={outlier_norms_output.mean() - outlier_norms_input.mean():+.4f})")
                    print(f"  Normal:  {normal_norms_input.mean():.4f} -> {normal_norms_output.mean():.4f} (Δ={normal_norms_output.mean() - normal_norms_input.mean():+.4f})")
                    print(f"  Input Gap: {(outlier_norms_input.mean() - normal_norms_input.mean()):.4f} | Output Gap: {(outlier_norms_output.mean() - normal_norms_output.mean()):.4f}\n")
    ...
    *,
    ...
    with torch.inference_mode():
        if do_light:
            ...
            if num_traj > 0:
                # Generate fixed U and noise with deterministic seed
                if fixed_eval_u is None or fixed_eval_u.shape[0] < num_traj:
                    gen = torch.Generator(device=device)
                    gen.manual_seed(args.seed + 999)
                    fixed_eval_u = torch.rand(num_traj, 1, device=device, generator=gen)
                ...
                # Use seeded generator for direction vectors
                gen_dir = torch.Generator(device=device)
                gen_dir.manual_seed(args.seed + 1000)
                uni = torch.randn(size=(num_traj, args.dim), device=device, generator=gen_dir)
                ...
                if args.target_dataset == 'funnel':
                    xlim = (-20.0, 20.0)
                    ylim = (-100.0, 100.0)
                elif args.target_dataset == 'unigauss':
                    xlim = (-100.0, 100.0)
                    ylim = (-100.0, 100.0)
                else:
                    xlim = (-4.0, 4.0)
                ...
                if n_paths > 0:
                    x_traj = odeint(ode_func, noise[:n_paths], t_vals, method='dopri5')
                    X = x_traj.detach().cpu().numpy()
                    starts = X[0]
                    ends = X[-1]
                    
                    # Identify outlier trajectories (top 2% by norm of starting noise)
                    noise_norms = torch.linalg.norm(noise[:n_paths], dim=1).cpu().numpy()
                    outlier_percentile = 2
                    outlier_count = max(1, int(n_paths * outlier_percentile / 100.0))
                    outlier_threshold = np.percentile(noise_norms, 100 - outlier_percentile)
                    outlier_indices = np.where(noise_norms >= outlier_threshold)[0]
                    
                    # Detailed logging - INPUT NOISE
                    outlier_norms_input = noise_norms[outlier_indices]
                    normal_norms_input = noise_norms[~np.isin(np.arange(n_paths), outlier_indices)]
                    
                    print(f"[Quantile @ step {step}] INPUT NOISE DISTRIBUTION")
                    print(f"  Total: {n_paths} | Outliers: {len(outlier_indices)} ({100*len(outlier_indices)/n_paths:.2f}%) | Normal: {n_paths - len(outlier_indices)}")
                    print(f"  Outlier norms - Mean: {outlier_norms_input.mean():.4f} | Std: {outlier_norms_input.std():.4f} | Min: {outlier_norms_input.min():.4f} | Max: {outlier_norms_input.max():.4f}")
                    print(f"  Normal norms  - Mean: {normal_norms_input.mean():.4f} | Std: {normal_norms_input.std():.4f} | Min: {normal_norms_input.min():.4f} | Max: {normal_norms_input.max():.4f}")
                    print(f"  Threshold: {outlier_threshold:.4f} | Gap (outlier - normal mean): {(outlier_norms_input.mean() - normal_norms_input.mean()):.4f}")
                    ...
                    if args.target_dataset in ['studentt']:
                        plt.rcParams['mathtext.fontset'] = 'cm'
                        plt.rcParams['font.family'] = 'serif'
                        
                        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
                        
                        # PLOT 1: Das Zentrum (Linear, stark reingezoomt)
                        # Beweist, dass der "Bulk" (die Hauptmasse) nicht kollabiert ist
                        ax1.scatter(ends[:, 0], ends[:, 1], s=3, alpha=0.4, color='#1f77b4', edgecolors='none')
                        ax1.set_xlim(-15, 15)
                        ax1.set_ylim(-15, 15)
                        ax1.set_aspect('equal', adjustable='box')
                        ax1.set_title("Generated Bulk (Linear Scale, Zoomed)", fontsize=16, pad=10)
                        ...
                        # PLOT 2: Die Ausreißer (SymLog Skala)
                        # Beweist, dass das Modell gigantische Extremwerte generiert
                        ax2.scatter(ends[:, 0], ends[:, 1], s=3, alpha=0.4, color='#d62728', edgecolors='none')
                        ax2.set_xscale('symlog', linthresh=2.0)
                        ax2.set_yscale('symlog', linthresh=2.0)
                        # Hinweis: set_aspect('equal') verträgt sich oft schlecht mit symlog, daher lassen wir es hier weg
                        ax2.set_title("Generated Heavy Tails (SymLog Scale)", fontsize=16, pad=10)
                        ...
                        plt.suptitle(f"Heavy Tail Analysis @ Step {step}", fontsize=18)
                        plt.tight_layout()
                        
                        # Speichern & Loggen
                        if args.runs_dir:
                            import os
                            ht_dir = os.path.join(args.runs_dir, "heavy_tail_analysis")
                            os.makedirs(ht_dir, exist_ok=True)
                            fig.savefig(os.path.join(ht_dir, f"heavy_tails_step_{step:06d}.png"), dpi=300)
                        
                        import wandb
                        if wandb.run is not None:
                            wandb.log({"eval/heavy_tails_2d": wandb.Image(fig)}, step=step)
                    ...
                    print(f"\n[Quantile @ step {step}] GENERATED OUTPUT DISTRIBUTION")
                    print(f"  Outlier norms - Mean: {outlier_norms_output.mean():.4f} | Std: {outlier_norms_output.std():.4f} | Min: {outlier_norms_output.min():.4f} | Max: {outlier_norms_output.max():.4f}")
                    print(f"  Normal norms  - Mean: {normal_norms_output.mean():.4f} | Std: {normal_norms_output.std():.4f} | Min: {normal_norms_output.min():.4f} | Max: {normal_norms_output.max():.4f}")
                    print(f"  Gap (outlier - normal mean): {(outlier_norms_output.mean() - normal_norms_output.mean()):.4f}")
                    
                    # COMPARISON: Input vs Output
                    print(f"\n[Quantile @ step {step}] INPUT -> OUTPUT TRANSFORMATION")
                    print(f"  Outlier: {outlier_norms_input.mean():.4f} -> {outlier_norms_output.mean():.4f} (Δ={outlier_norms_output.mean() - outlier_norms_input.mean():+.4f})")
                    print(f"  Normal:  {normal_norms_input.mean():.4f} -> {normal_norms_output.mean():.4f} (Δ={normal_norms_output.mean() - normal_norms_input.mean():+.4f})")
                    print(f"  Input Gap: {(outlier_norms_input.mean() - normal_norms_input.mean()):.4f} | Output Gap: {(outlier_norms_output.mean() - normal_norms_output.mean()):.4f}\n")
                    ...
                    if args.target_dataset == "radialpareto":
                        plot_tail_ccdf(args, ends_norms, step=step, filename="tail_ccdf", key="quantile/tail_ccdf")
