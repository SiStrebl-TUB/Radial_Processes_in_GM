...
# --- Parameter ---
STD = 0.3  # Muss mit dem Sampler übereinstimmen!
MEANS = [-1, 1]
WEIGHTS = [0.5, 0.5]

def bimodal_cdf(x, std=STD):
    """Die exakte analytische CDF der Mischverteilung."""
    cdf1 = norm.cdf(x, loc=MEANS[0], scale=std)
    cdf2 = norm.cdf(x, loc=MEANS[1], scale=std)
    return WEIGHTS[0] * cdf1 + WEIGHTS[1] * cdf2

def get_true_quantile(u, std=STD):
    """
    Berechnet das exakte Quantil numerisch durch Invertieren der CDF.
    Löst: CDF(x) - u = 0
    """
    # Funktion, deren Nullstelle wir suchen: CDF(x) - Ziel_u
    target_func = lambda x: bimodal_cdf(x, std) - u
    
    # Brent-Methode ist sicher und schnell für Root-Finding in einem Intervall
    # Wir suchen x zwischen -5 und 5 (da spielt sich alles ab)
    try:
        return brentq(target_func, -5, 5)
    except ValueError:
        return np.nan # Falls numerisch was schief geht (z.B. bei u=0 oder u=1)
...
def pretrain_quantile(args, *, log_step_offset: int = 0) -> Tuple[nn.Module, str, int]:
    ...
    dim = args.dim
    sampler = smpl.get_distribution("quantile_only")
    ...
    for step in tqdm(range(total_steps), desc="Quantile training (ot)"):
        ...
        # only need positive noise for radial approach
        #eps = torch.abs(eps)
        #dqdt = torch.abs(dqdt)
        ...
        if step in [0, 10000, 50000, 100000]: #[0, 9999, 29999, 49999, 79999, 99999, 199999]:
            #u_test = torch.linspace(0.01, 0.99, B, device = device)
            #u_test = u_test.reshape(B, 1)
            u_grid = torch.concatenate([
                torch.linspace(0.001, 0.45, 500),
                torch.linspace(0.45, 0.55, 1000), # Hohe Auflösung am Sprung
                torch.linspace(0.55, 0.999, 500)
            ])
            u_grid,_ = torch.sort(u_grid)
            u_test = u_grid.reshape(-1,1)
            t_test = torch.ones(2000, 1, device = device)
            eps_test, dqdt_test = quantile(u_test, t_test, return_dqdt=True, requires_grad = False)

            dqdt_test = dqdt_test.detach().cpu().numpy()
            u_test = u_test.detach().cpu().numpy()
            plt.plot(u_test[:,0], dqdt_test[:,0], label = f"quantile at step {step}")
            plt.legend()
            plt.grid(True)
            plt.savefig('quantiles')
        if step == 100000:
            x0_quantile = [get_true_quantile(u) for u in u_test[:,0]]
            plt.plot(u_test[:,0], x0_quantile, label = "target quantile")
            plt.legend()
            plt.grid(True)
            plt.savefig('quantiles')
        ...
        transport_loss, transport_metrics = transport_objective(x0, dqdt_1d)
        reg_logdet = (
