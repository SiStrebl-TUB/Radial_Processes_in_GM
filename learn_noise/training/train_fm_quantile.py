...
def get_quantile(args, device):
    if args.mode == 'pretrain_quantile':
        ...
        quantile = build_quantile(args, device, dim = 1) #args.dim
...
def train_fm_quantile(
    ...
    for step in tqdm(range(args.epochs), desc="Flow-matching (test trajectories)"):
        ...
        u = u_eps + (1 - 2 * u_eps) * torch.rand(x_0.shape[0], 1, device=device, dtype=x_0.dtype) #args.dim
        ...
        if args.use_minibatch_ot:
            with torch.no_grad():
                ...
                Q_no_grad = quantile(u, t_1).reshape(x_0.shape[0], 1)
                Q_no_grad = Q_no_grad * uni
        ...
        x_t_cond0 = eps*uni
        x_t = (1.0 - t) * x_0 + x_t_cond0
        target_velocity = -x_0 + dqdt * uni
