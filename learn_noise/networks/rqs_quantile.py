...
class RQSQuantile(nn.Module):
    ...
    def _compute_quantile_and_jac(
        ...
        *,
        ...
        # 1. Das rohe (noch eventuell negative) Quantil berechnen
        q_raw = z_curr * scale.view(1, dim) + self.bias.view(1, dim)
        
        # 2. NEU: Softplus anwenden, um den Radius strikt positiv zu machen!
        q = torch.nn.functional.softplus(q_raw)

        if return_dq_du and dydz_total is not None and dz_du is not None:
            # 3. Die rohe Ableitung berechnen (wie bisher)
            dq_raw_du = (scale.view(1, dim) * dydz_total) * dz_du
            
            # 4. NEU: Die Kettenregel für Softplus anwenden! 
            # d(softplus(x))/dx = sigmoid(x)
            dq_du = dq_raw_du * torch.sigmoid(q_raw)
