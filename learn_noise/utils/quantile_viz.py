...
def log_noise_slices(quantile, device, step, times=(0.0, 0.03, 0.1, 0.15, 0.25, 0.5, 0.75, 0.9, 0.98, 1.0), N=4000, dim=2, u_eps=5e-5):
    ...
    for ax, tt in zip(axes, times):
        U = torch.rand(N, 1, device=device).clamp(u_eps, 1 - u_eps) #dim
        tau = torch.full((N, 1), float(tt), device=device)
        eps = quantile(U, tau)  # (N,dim)
        uni = torch.randn(size = (N, dim), device = device)
...
def log_xt_slices(quantile, sampler, device, step, times=(0.0, 0.1, 0.25, 0.5, 0.75, 0.9, 1.0), N=4000, dim=2, u_eps=5e-5):
    ...
    eps1 = quantile(U, torch.ones(N, 1, device=device), asRad = True)
    uni = torch.randn(size = (N,dim), device = device)
    uni = torch.nn.functional.normalize(uni)
    eps1 = eps1*uni
...
def log_noise_slices_fixed(
    ...
    U_all = None
    if share_u:
        U_all = [torch.rand(Nu, 1, device=device).clamp(u_eps, 1 - u_eps) for _ in range(K)] #dim
    ...
    for k in range(K):
        ...
        ref_pts = []
        for tt in times:
            U_tmp = (U_all[k] if share_u else torch.rand(Nu, 1, device=device).clamp(u_eps, 1 - u_eps)) #dim
            tau = torch.full((Nu, 1), float(tt), device=device)
            eps = quantile(U_tmp, tau, asRad = True)
            uni = torch.randn(size = (Nu, dim), device = device)
            uni = torch.nn.functional.normalize(uni)
            eps = eps*uni
        ...
        for j, tt in enumerate(times):
            ...
            U = U_all[k] if share_u else torch.rand(Nu, 1, device=device).clamp(u_eps, 1 - u_eps) #dim 
            tau = torch.full((Nu, 1), float(tt), device=device)
            eps = quantile(U, tau, asRad = True)
            uni = torch.randn(size = (Nu,dim), device = device)
            uni = torch.nn.functional.normalize(uni)
